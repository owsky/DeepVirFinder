...
import numpy as np
import sklearn
from keras.layers import Dense, Dropout, Input, Conv1D, GlobalMaxPooling1D, Average
...
# validation
filename_codevalfw = \
    [x for x in os.listdir(inDirVal) if 'codefw.npy' in x and 'virus' in x and contigLengthk + 'k' in x][0]
...
# training
filename_codetrfw = [x for x in os.listdir(inDirTr) if 'codefw.npy' in x and 'host' in x and contigLengthk + 'k' in x][
    0]
print("data for training " + filename_codetrfw)
...
# validation
filename_codevalfw = \
    [x for x in os.listdir(inDirVal) if 'codefw.npy' in x and 'host' in x and contigLengthk + 'k' in x][0]
print("data for validation " + filename_codevalfw)
...
modPattern = 'model_siamese_varlen_' + contigLengthk + 'k_fl' + str(filter_len1) + '_fn' + str(
    nb_filter1) + '_dn' + str(nb_dense)
