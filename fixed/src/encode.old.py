    ...
    for line in faLines:
        if flag == 0 and line[0] == '>':
            ...
            while posEnd <= len(seqCat):
                contigName = contigType + "#" + NCBIName + "#" + str(contigLengthk) + "k#" + head.split('/')[
                    -1] + "#" + str(pos) + "#" + str(posEnd)
                ...
                if len(seqname) > 0 and len(seqname) % 4000000 == 0:
                    ...
                    fileCount += 1
                    codeFileNamefw = contigType + "#" + NCBIName + "#" + str(contigLengthk) + "k_num" + str(
                        fileCount) + "_seq" + str(len(code)) + "_codefw.npy"
                    codeFileNamebw = contigType + "#" + NCBIName + "#" + str(contigLengthk) + "k_num" + str(
                        fileCount) + "_seq" + str(len(codeR)) + "_codebw.npy"
                    nameFileName = contigType + "#" + NCBIName + "#" + str(contigLengthk) + "k_num" + str(
                        fileCount) + "_seq" + str(int(len(seqname) / 2)) + ".fasta"
...
if len(code) > 0:
    codeFileNamefw = contigType + "#" + NCBIName + "#" + str(contigLengthk) + "k_num" + str(
        fileCount + 1) + "_seq" + str(len(code)) + "_codefw.npy"
    codeFileNamebw = contigType + "#" + NCBIName + "#" + str(contigLengthk) + "k_num" + str(
        fileCount + 1) + "_seq" + str(len(codeR)) + "_codebw.npy"
    nameFileName = contigType + "#" + NCBIName + "#" + str(contigLengthk) + "k_num" + str(fileCount + 1) + "_seq" + str(
        int(len(seqname) / 2)) + ".fasta"
    print("encoded sequences are saved in {}".format(codeFileNamefw))
    np.save(os.path.join(outDir, codeFileNamefw), np.array(code))
    np.save(os.path.join(outDir, codeFileNamebw), np.array(codeR))
    seqnameF = open(os.path.join(outDir, nameFileName), "w")
