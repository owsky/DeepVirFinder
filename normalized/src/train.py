...
parser.add_option("-s", "--len0", action = "store", type = int, dest = "contigLength0", help = "contig Length")
parser.add_option("-l", "--len", action = "store", type = int, dest = "contigLength", help = "contig Length")
...
if (options.contigLength is None or
    ...
    options.nb_filter1 is None or 
    options.nb_dense is None or 
    options.contigLength0 is None) :
...
contigLength0 = options.contigLength0
...
contigLengthk0 = contigLength0/1000
if contigLengthk0.is_integer() :
    contigLengthk0 = int(contigLengthk0)

contigLengthk0 = str(contigLengthk0)
...
def adjust_seq(seq, seq0):
    global contigLength, contigLength0
    padding_length = contigLength - contigLength0
    seq0 = np.pad(seq0, ((0,0),(0,padding_length),(0,0)), mode='constant')
    return np.concatenate((seq, seq0), axis=0)
...
# training
filename_codetrfw = [ x for x in os.listdir(inDirTr) if 'codefw.npy' in x and 'virus' in x and contigLengthk0 + 'k' in x ][0]
print("data for training " + filename_codetrfw)
phageRef_codetrfw0 = np.load(os.path.join(inDirTr, filename_codetrfw))
phageRef_codetrbw0 = np.load(os.path.join(inDirTr, filename_codetrfw.replace('fw', 'bw')))
# validation
filename_codevalfw = [ x for x in os.listdir(inDirVal) if 'codefw.npy' in x and 'virus' in x and contigLengthk0 + 'k' in x ][0]
print("data for validation " + filename_codevalfw)
phageRef_codevalfw0 = np.load(os.path.join(inDirVal, filename_codevalfw))
phageRef_codevalbw0 = np.load(os.path.join(inDirVal, filename_codevalfw.replace('fw', 'bw')))

phageRef_codetrfw = adjust_seq(phageRef_codetrfw, phageRef_codetrfw0)
phageRef_codetrbw = adjust_seq(phageRef_codetrbw, phageRef_codetrbw0)
phageRef_codevalfw = adjust_seq(phageRef_codevalfw, phageRef_codevalfw0)
phageRef_codevalbw = adjust_seq(phageRef_codevalbw, phageRef_codevalbw0)
...
# training
filename_codetrfw = [ x for x in os.listdir(inDirTr) if 'codefw.npy' in x and 'host' in x and contigLengthk0 + 'k' in x ][0]
print("data for training " + filename_codetrfw)
hostRef_codetrfw0 = np.load(os.path.join(inDirTr, filename_codetrfw))
hostRef_codetrbw0 = np.load(os.path.join(inDirTr, filename_codetrfw.replace('fw', 'bw')))
# validation
filename_codevalfw = [ x for x in os.listdir(inDirVal) if 'codefw.npy' in x and 'host' in x and contigLengthk0 + 'k' in x ][0]
print("data for validation " + filename_codevalfw)
hostRef_codevalfw0 = np.load(os.path.join(inDirVal, filename_codevalfw))
hostRef_codevalbw0 = np.load(os.path.join(inDirVal, filename_codevalfw.replace('fw', 'bw')))
